...
class LaboratoryBusinessWorkflowTests(TestCase):
    ...
    def test_complete_patient_to_payment_workflow(self):
        ...
        self.assertEqual(technician_approve_response.status_code, 400)
        ...
        self.assertEqual(pathologist_submit_response.status_code, 400)
        self.authenticate_as(self.lab_technician)
...
class LaboratoryValidationTests(TestCase):
    ...
    def test_result_creation_uses_only_active_test_parameters_and_service_retrieval(self):
        inactive_parameter = TestParameterService.create_parameter(
            laboratory_test=self.laboratory_test,
            name="Inactive Parameter",
            display_order=2,
        )
        inactive_parameter.is_active = False
        inactive_parameter.save(update_fields=["is_active"])
        sample = Sample.objects.create(
            sample_id="SAM-ACTIVE-PARAMETERS", visit=self.visit, sample_type="BLOOD", status=SampleService.COLLECTED,
        )
        ordered_test = OrderedTest.objects.create(
            order_id="ORD-ACTIVE-PARAMETERS", visit=self.visit, laboratory_test=self.laboratory_test,
            sample=sample, status="SAMPLE_COLLECTED",
        )
        result = ResultService.create_result(order_id=ordered_test.order_id)
        self.assertEqual(result.parameters.count(), 1)
        self.assertEqual(ResultService.get_result(result_id=result.result_id), result)
