...
class BillingIntegrityServiceTests(TestCase):
    ...
    def test_only_successful_payments_can_be_refunded(self):
        invoice = self.create_finalized_invoice()
        payment = PaymentService.record_payment(invoice=invoice, amount=Decimal("10.00"), payment_method="CASH")
    ...
    def test_invoice_pdf_download_and_public_verification(self):
        invoice = self.create_draft_invoice_with_test()
        client = APIClient(); client.force_authenticate(self.billing_user)
        response = client.get(f"/api/billing/{invoice.invoice_id}/download/")
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response["Content-Type"], "application/pdf")
        self.assertTrue(response.content.startswith(b"%PDF"))
        verification = client.get(f"/api/billing/verify/{invoice.verification_token}/")
    ...
    def test_discount_is_persisted_before_totals_are_calculated(self):
        invoice = self.create_draft_invoice_with_test()
        second_test = InvoiceItem.objects.create(
            invoice=invoice,
            item_type=InvoiceItem.ItemType.TEST,
            item_id="TEST-SECOND",
            item_name="Second test",
            quantity=1,
            unit_price=Decimal("50.00"),
            line_total=Decimal("50.00"),
        )
        self.assertIsNotNone(second_test.pk)
        InvoiceService.calculate_totals(invoice)

        invoice = InvoiceService.apply_discount(invoice=invoice, discount=Decimal("10.00"))
