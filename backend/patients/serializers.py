...
class PatientListQuerySerializer(serializers.Serializer):
    ...
    ordering = serializers.ChoiceField(
        choices=("registered_on", "-registered_on", "full_name", "-full_name"),
        required=False,
        default="-registered_on",
    )

    def validate(self, attrs):
        if attrs.get("registered_from") and attrs.get("registered_to"):
            if attrs["registered_from"] > attrs["registered_to"]:
                raise serializers.ValidationError(
                    "registered_from cannot be after registered_to."
                )
        return attrs
