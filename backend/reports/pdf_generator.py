...
class ReportPDFGenerator:
    ...
    @staticmethod
    def generate(report_data, include_header=True, verification_url=None):
        ...
        if verification_url:
            ...
            elements.append(Paragraph("Scan to verify this report:", styles["Label"]))
            qr = qr_image(verification_url)
            elements.append(Image(qr, width=72, height=72))
            elements.append(Paragraph(ReportPDFGenerator._text(verification_url), styles["NormalCell"]))
#-----------------
        build_kwargs = {"canvasmaker": NumberedCanvas}
        if include_header:
            build_kwargs.update(
                onFirstPage=ReportPDFGenerator.draw_letterhead,
                onLaterPages=ReportPDFGenerator.draw_letterhead,
            )
        doc.build(elements, **build_kwargs)
            ...
            rows.append(
                [
                    ...
                    Paragraph(
                        ...
                        if parameter.flag in {parameter.Flag.HIGH, parameter.Flag.LOW}
                        else ReportPDFGenerator._text(value),
                        getSampleStyleSheet()["BodyText"],
                    ),
                    Paragraph(
